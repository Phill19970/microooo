...
import com.capstone.medicalrecordservice.dto.v1.ExceptionDTO;
...
class ControllerAdviceTest {
    ...
    public void testMedicalRecordExists() {
        // Arrange
        MedicalRecordExists exception = new MedicalRecordExists("Medical Record exists");

        // Act
        ExceptionDTO response = controllerAdvice.medicalRecordExists(exception);

        // Assert
        assertThat(HttpStatus.CONFLICT).isEqualTo(response.getStatus());
        assertThat("Medical Record exists").isEqualTo(response.getDetails());
        assertThat(LocalTime.now().getHour()).isEqualTo(response.getTimestamp().getHour());
    }
